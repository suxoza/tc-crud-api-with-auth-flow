...
const SignUp = () => {
    ...
    const submitForm = (data: User) => {
        setError(false)
        dispatch(registerUser(data) as any).then((responce: any) => {
            if(responce?.error)
                setError(true)
        })
    }
    ...
    return (
        <>
            <div id="SignUpContaiener" className="relative flex flex-col justify-center min-h-screen overflow-hidden">
                {error && <div className="flex items-center justify-center">
                    <div className="mt-10 w-1/3 p-2 bg-red-400 items-center text-indigo-100 leading-none lg:rounded-full flex justify-center" role="alert">
                        <span className="font-semibold mr-2 flex-auto p-3">UserEmail already in use</span>
                    </div>
                </div>}
                <div className="w-full mt-3 p-6 m-auto bg-white rounded-md shadow-md lg:max-w-xl">
                    ...
                    <form className="mt-6" onSubmit={handleSubmit(submitForm)}>
                        <div className="mb-2">
                            ...
                            <input {...register('name')} required type="text" className="block w-full px-4 py-2 mt-2 text-purple-700 bg-white border rounded-md focus:border-purple-400 focus:ring-purple-300 focus:outline-none focus:ring focus:ring-opacity-40"/>
                        </div>
                        <div className="mb-2">
